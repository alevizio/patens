...
var getPythonProgress = () => lastProgress;
...
var ensurePython = () => {
	...
	pyodidePromise = (async () => {
		...
		setProgress({
			...
			message: "Installing fontTools & brotli…"
		});
...
var otfToWoff2 = async (otfBuffer) => {
...
var compileFeaIntoFont = async (otfBuffer, feaSource) => {
	const py = await ensurePython();
	py.FS.writeFile("/tmp/in.otf", new Uint8Array(otfBuffer));
...
*/
var finalizeFont = async (otfBuffer, opts) => {
...
*/
var instancesAsStaticZip = async (input) => {
	const py = await ensurePython();
	const fileNames = [];
...
*/
var buildVariableFont = async (input) => {
	const py = await ensurePython();
	const fileNames = [];
	for (let i = 0; i < input.masters.length; i++) {
		const fname = `/tmp/master_${i}.otf`;
...
*/
var projectToUfoZip = async (projectJson, familyName) => {
	const py = await ensurePython();
...
var ufoZipToProject = async (zipBuffer) => {
	const py = await ensurePython();
...
`;
//#endregion
export { getPythonProgress as a, projectToUfoZip as c, finalizeFont as i, ufoZipToProject as l, compileFeaIntoFont as n, instancesAsStaticZip as o, ensurePython as r, otfToWoff2 as s, buildVariableFont as t };
