...
${project.brief?.intent?.trim() || project.description?.trim() || 'A typeface exported from Font Studio.'}
...
- \`${safeFilename(project.name)}.font.json\` — Full project state for re-import into Font Studio.
...
_Generated by Font Studio on ${new Date().toISOString().slice(0, 10)}._
`;
		downloadBlob(
