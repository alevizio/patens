			...
			items: [
				{
					...
					a: 'The audit module is the headline feature — a continuous integrity + teaching check across 101 codes spanning contour shape, vertical metrics, OpenType invariants, brief completeness, multi-script coverage, kerning hygiene, and anchor naming. Every code carries plain-English prose explaining what it means + why it matters, and around 30 codes have a one-click "Fix" button that applies the auto-fix. Errors block release; warnings are advice. Click "All" next to an issue to see every glyph with the same audit code. The same 102-code engine also runs from a terminal via `npx patens audit my.font.json` — drop it into your CI as a font-design lint step.'
				},
				{
