...
//   FIT_FONT_URL    — private Blob URL of Fit.woff2 (in the patens-fonts store)
//   FIT_FONT_TOKEN  — Blob read token for the SAME store as FIT_FONT_URL
//
// Note: the auto-injected BLOB_READ_WRITE_TOKEN on a Vercel project can
// belong to a different Blob store than where Fit lives, so we use a
// dedicated env var that's pinned to the right store.
//
...
const token = process.env.FIT_FONT_TOKEN;
...
if (!url || !token) {
	...
	console.warn(
		'[fetch-fit] FIT_FONT_URL or FIT_FONT_TOKEN not set and no local Fit.woff2. The hero will fall back to ui-sans-serif.'
	);
	process.exit(0);
}
