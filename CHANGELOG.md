...
- **Citation engine MVP at `src/lib/citations/`** (5 files + 12 tests) — Feature 1 of `docs/research/ai-features-roadmap.md` going live. `CITATION_MAP` populated with anchor + gist data for 50+ codes drawn from the 10-source open-licensed MVP corpus. 49 of 94 `/audit/[code]` prerendered pages now ship a "Canonical references" section. Bibliographic citations only (no body-text ingest); craft-canon citations (Tracy/Smeijers/Noordzij/Cheng) flagged for license-required ingestion in Q3 2026.
