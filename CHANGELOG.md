...
- **Research foundation at `docs/research/`** — 4 docs totaling ~1900 lines:
  - `canonical-library.md` (526 lines, 38 sources) — RAG corpus for the citation engine. Licensing matrix per audit family. 10-source open-licensed MVP starter corpus covering all 9 families.
  - `ai-audit-mapping.md` (415 lines) — All 94 codes classified for AI augmentation: 75 algorithm-only, 11 vision-augmented, 1 vision-primary (`sharp-kink`), 3 LLM-augmented. Top 10 vision second-opinion candidates with prompt templates. ~$100/weekend empirical methodology.
  - `multi-script-canon.md` (691 lines) — Cyrillic, Greek, Arabic, Hebrew, Devanagari, CJK, Thai, Armenian, Georgian, Tibetan with native-tradition citations. Recommends Cyrillic + Greek + Arabic as 3-script MVP.
  - `ai-features-roadmap.md` (285 lines) — 5-feature product roadmap with launch-gate ordering, dependencies, grant-application leverage. Synthesizes the three research tracks.
- **5 paste-ready grant applications** at `docs/launch/`:
  - `anthropic-application.md` (203 lines) — combined Startup + Research Credits, $50K target. Leads with the 75/11/1/3 audit classification.
  - `github-secure-oss-application.md` (232 lines) — $10K + hardening sprint. Five concrete sprint deliverables (SLSA, fuzz, reproducible builds, CLI hardening, threat model doc).
  - `futo-microgrant-application.md` (77 lines) — $5K cash, anti-establishment voice, $5K line-itemed against bespoke Cyrillic + Greek lowercase + TypeCon travel.
  - `mozilla-moss-application.md` (171 lines) — $25K–$50K cash. Five-axis mission alignment (open web, privacy, accessibility, open source, multi-script inclusivity).
- **Pre-launch master runbook** at `docs/launch/master-runbook.md` (300 lines) — 10-week calendar from Jun 2 to Aug 8 with per-week tasks + ownership (you vs me). Critical-path items explicitly flagged.
- **Show HN day hourly runbook** at `docs/launch/show-hn-day-runbook.md` (247 lines) — sleep-deprived cockpit-checklist for Tuesday August 4, 2026. Hour-by-hour from 04:45 AM PT wake-up through 21:00 PM PT bedtime handoff.
- **Handoff log template** at `docs/launch/handoff-template.md` — copy per milestone day (`handoff-YYYY-MM-DD.md`) for tracking submissions, metric snapshots, conversations, questions, bugs, surprises.
- **TypeCon Portland 2026 intel** at `docs/launch/typecon-portland-2026.md` (5,150 words) — 2026 CFP CLOSED (Apr 6 extended deadline; notifications early May). Top-5 networking targets: Dave Crossland (Google Fonts), Glenda Bellarosa (Adobe Fonts / Font Bakery), Lizy Gershenzon + Travis Kochel (Future Fonts), Christopher Slye (SOTA Ex Officio), Neil Summerour (SOTA Treasurer / Positype). Three drafted talk proposals ready for 2027 CFP. Budget $3,750 mid-case all-in.
- **Press contacts research** at `docs/launch/press-contacts.md` (5,678 words) — Tier-1 lists 15 type-design venues (Typographica, ILT, Eye, AIGA, Slanted, Type.today, Alphabettes, Print, Daily Heller) with editor names + verified contact paths. Tier-2 covers design + tech pubs. Section 3 profiles 12 specific journalists. Three outreach templates + T-22 → T+62 calendar.
- **TypeCon Top-5 outreach emails** at `docs/launch/typecon-outreach-emails.md` — paste-ready emails for the 5 highest-leverage TypeCon attendees. Send target Tue Jul 15, 2026 (3 weeks pre-conference). Tracking template + follow-up policy + Tier-2 backup list.
- **Show HN day hourly runbook** at `docs/launch/show-hn-day-runbook.md` — sleep-deprived cockpit checklist for Tuesday August 4, 2026.
- **Vision experiment harness** at `scripts/vision-experiment.mjs` (353 lines) — runnable dry-run validation of the methodology from `ai-audit-mapping.md` Section 3. 100 entries (10 codes × 10 fonts), $4 cost-estimate baseline. Anthropic SDK wiring stubbed; deferred to credits-landing per `anthropic-application.md`.
- **Citation engine MVP at `src/lib/citations/`** (5 files + 12 tests) — Feature 1 of `docs/research/ai-features-roadmap.md` going live. `CITATION_MAP` populated with anchor + gist data for 91 of 101 codes drawn from the 11-source open-licensed MVP corpus (OpenType spec, TrueType ref, AGL, FEA, UFO 3, Unicode 16, Sheep CC, OT Cookbook, Klim, Knuth 1979, variablefonts.io, Frere-Jones blog). 93 of 101 `/audit/[code]` prerendered pages ship a "Canonical references" section. Bibliographic citations only (no body-text ingest); craft-canon citations (Tracy/Smeijers/Noordzij/Cheng) flagged for license-required ingestion in Q3 2026.
- **Variable-fonts research foundation** at `docs/research/`:
  - `variable-fonts-deep-dive.md` (571 lines, 8 parts, 50+ sources) — Origin + history of variable fonts (Multiple Master 1990s → OpenType 1.8 2016), the 5 registered axes (wght/wdth/slnt/ital/opsz), custom axes (GRAD/XOPQ/softness), the variations math (gvar/avar/STAT/HVAR/MVAR), designspace concept, VF support across 11 type editors, designer-side considerations, multi-script implications, ranked 10-feature roadmap.
  - `variable-fonts-v2-implementation-plan.md` (398 lines) — Paste-ready specs for the 6 pre-launch + 4 v2 audit codes with check/fix pseudocode, citation mappings ready for matcher.ts, STAT editor UI specification, designspace round-trip module specification, file-list-to-update for each scenario, 3-week sprint plan if all priorities ship pre-launch.
- **5 new variable-font audit codes shipped (94 → 99) + Stat type model**:
  - `axis-range-extreme` (info) — flags wght > 800, wdth > 100, opsz > 50pt, slnt > 30° threshold violations. Custom axes (uppercase tags like GRAD) intentionally excluded.
  - `master-too-close` (warn) — pairwise normalized euclidean distance between masters across shared axes. Threshold 5%.
  - `stat-missing` (warn) — variable font has axes but no `familyAxes` set on the project (Patens uses familyAxes to generate STAT at export-time).
  - `stat-format-mismatch` (warn) — STAT italic axis-value uses format 1 instead of format 3 (linkedValue). Format 1 breaks Windows style-name composition ("Regular Bold Italic" instead of "Bold Italic").
  - `stat-instance-name-mismatch` (warn) — STAT-composed instance name differs from the fvar `styleName` for the same location. Causes inconsistent display across OS apps.
  - Each with full describeAuditCode prose, citation mapping (OpenType STAT chapter + variablefonts.io primer), check function in preflightProject(), and 17 new unit tests across positive/negative/edge cases.
  - New `Stat` type model in `types.ts` — DesignAxisRecord + 4-format AxisValue discriminated union (Format 1/2/3/4) per the OpenType STAT spec. Project gains optional `stat?: Stat` field; undefined = let Patens generate minimal STAT from fvar at export.
- **Designspace v5 import + export** at `src/lib/font/designspace.ts` — round-trip XML interchange with Glyphs, FontLab, RoboFont, and Fontmake.
  - `parseDesignspaceXml(xml)` — parses axes + sources + instances; validates 4-char axis tags, min/default/max ordering, non-empty axes. DOMParser-based; browser-only.
  - `designspaceToProject(ds, existing?)` — maps the structured shape to Patens Axis / Master / VariableInstance records. Identifies the default source (matching every axis default) and maps non-default sources to Project.masters[].
  - `designspaceFromProject(project)` — serializes back to v5 XML. Emits default-location source plus non-default master sources, instances with familyName/styleName/postScriptName, axes. Includes XML attribute escaping.
  - 21 tests covering parser correctness, mapping logic, error paths, XML escaping, and the round-trip lossless guarantee.
  - **UI wiring complete end-to-end**: Studio dashboard "Import from .designspace" button creates a fresh project from the XML; `/project/[id]/export` Import panel merges into the current project; `/project/[id]/export` Export panel button (disabled when no axes) downloads back to `.designspace`. Dynamic imports keep the parser chunk off the cold-load path.
...
- **Second Swiss editorial wave across the deferred surfaces.** `/audit/[code]` (94 dynamic SEO pages — auto-fix pill + CLI code block + peer-link tiles + kbd + code chips rectilinear). `/privacy` + `/security` (5 numbered sections each + rectilinear inline `<code>` chips + focus-visible back-links). `+error.svelte` (tofu codepoint boxes go rectilinear, reinforcing the "glyph missing from typeface" metaphor). `/learn/*` (7 tutorial pages — step badges + code chips + bullet dots + TOC nav). `/share/[id]` (50 chrome instances stripped on the most-public alpha surface). `/studio-c104c94c` + `/family/[id]` (24 each, the alpha dashboard + family hub).
- **`/es` structural numbered prefixes.** 5 multi-section Spanish pages (`/es/audit`, `/es/press`, `/es/privacy`, `/es/security`, `/es/about`) gain mono 01–0X H2 prefixes matching the EN pattern. Pages with 1–2 H2s skipped (no rhythm to establish). Voseo voice preserved verbatim.
- **Global `max-w-[60ch]` constraint on body prose.** One CSS rule in `app.css` applies the Swiss-spec body-text width to every `<p class="...leading-relaxed...">` page-wide. `:not([class*='max-w-'])` guard preserves explicit per-paragraph designer intent on the rare paragraphs that need to span wider.
- **Plex Sans CSS async-loaded.** Saves 100–300ms LCP on cold loads via the canonical Filament Group pattern (`media=print + onload=media=all`); NoScript fallback preserves render-blocking behavior for non-JS users.
- **OG card unused color const removed.** Dropped the `SUBTLE` palette constant in `og.png/+server.ts` left over from the pre-Swiss palette.
...
- **WaitlistForm now actually respects `prefers-reduced-motion`.** The comment claimed Svelte's `fly`/`fade` transitions auto-respect the OS setting; they don't. Svelte transitions are JS-driven (tween properties via tick callbacks) and bypass the global `@media (prefers-reduced-motion: reduce)` reset in app.css. Fixed by reading `prefersReducedMotion()` at transition time and zeroing the duration when motion is reduced.
- **EN home `/` declared canonical but no hreflang alternate to `/es`.** Google won't link the two locales without bidirectional declarations. Added via the existing `hreflangLinks('/')` helper so the pair is symmetric.
- **Sitemap lastmod stale post-Swiss pass.** Every page that received substantive layout changes bumped to 2026-05-30 lastmod — honest crawler signal that the editorial surface actually changed.
- **10 `svelte/no-at-html-tags` lint errors on `/es/*` pages.** The pages used `{@html hreflangLinks(...)}` without the canonical eslint-disable comment the EN counterparts have. The helper returns a static template string from `src/lib/i18n.ts` — known-safe to inject. Added the disable comment before each of the 10 hits.
- **Two typographic glitches per Swiss spec.** `/share/[id]` input placeholder `"Type something..."` → `"Type something…"` (ellipsis character). `/es/pronunciation` `"patent"` → `"patent"` (curly quotes around the English loan word).
